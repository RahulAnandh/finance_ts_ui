...
const DeployedTags: React.FC = () => {
  const tag = useAppSelector((state) => state.tag);
  const dispatch = useAppDispatch();
  const [form] = Form.useForm();
  const [data, setData] = useState(tag.tag_list.deployedTagDetails);
  const [editingKey, setEditingKey] = useState("");

  useEffect(() => {
    setData(tag.tag_list.deployedTagDetails);
  }, [tag.tag_list]);
  const isEditing = (record: DataType) => record.hwTagId === editingKey;

  const edit = (record: Partial<DataType>) => {
    console.log("1---1B", form, record);
    form.setFieldsValue({
      ...
      inputType === "select" ? (
        <Select
          options={
            ...
            tag.hardware_team_list.map((team) => {
